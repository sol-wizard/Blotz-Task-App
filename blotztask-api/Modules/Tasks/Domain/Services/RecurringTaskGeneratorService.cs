...
public class RecurringTaskGeneratorService
{
    ...
    public IReadOnlyList<TaskItem> GenerateInstances(RecurringTask template, DateOnly from, DateOnly to)
    {
        ...
        var current = from < template.StartDate ? template.StartDate : from;

        while (current <= to)
        {
            if (template.EndDate.HasValue && current > template.EndDate.Value)
                break;

            if (IsOccurrence(template, current))
                results.Add(CreateTaskItem(template, current));
