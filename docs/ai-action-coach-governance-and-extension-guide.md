...
| 对当前消息的语义解释候选 | Model |
| 引用来源、角色、文本跨度和映射是否合法 | Evidence Guard |
...
Verified Interpretation：由模型提出并通过来源/结构校验的当前 Turn 解释；不意味着语义被确定性证明。
Derived Decision：根据事实和 Policy 计算出来，不单独持久化的结果。
Versioned Policy：产品允许调整的策略参数。
Candidate Payload：尚未通过 Guard 和 Kernel 的模型候选。
```

不得因为多个模块都需要某个派生结果，就把它复制为多个布尔字段。优先传递一个强类型决策对象。`Facts`、`Phase`、`allowedActions` 若是权威实体的投影，由同一个函数生成并校验，不能另建三套状态机。

模型提议的 Item 分类、Disposition、Support Kind 都可能理解错误，即使 Quote 确实存在。来源校验只证明引用合法；需要持久保存的内容必须保留来源、作用域、可修正性和不确定性。正式副作用的授权只能来自结构化用户 Command，不能借助 `Verified` 命名绕过确认。
...
User Message 持久化
  ...
  -> 可选 Deterministic Proposal Generator 生成候选，并重新通过同一套 Response / Artifact Guards
...
- 保留回应当前拒绝、修正、暂停和新话题的普通对话出口；历史 Ready 状态不能只开放 Proposal。
...
- 验证引用文本确实存在于对应用户消息。
- 将通过来源/结构检查的解释转为 `VerifiedInterpretation`，保留其语义局限。
...
Post-Policy 的唯一决策顺序由 [技术方案 13.7](ai-action-coach-technical-design-v3.md#137-完整-policy-contract) 定义：先约束有效性与权限，再处理当前拒绝/修正和派生行为空间，最后检查候选及其 Payload。不能因为模型选错 Response 类型而先返回无关 fallback，忽略用户实际的停止请求。

后置 Guard 不通过时，把类型化失败报告交还同一 Post-Policy，或执行 StrategyDecision 已包含的恢复计划；Runtime 不按某个产品 Reason Code 自己选策略。恢复计划必须重新满足当前回应约束，提问不一定比卡片更适合用户。
...
- 确定性拒绝未允许的结构化 Action、非法引用和字段。正式成功文案由回执投影。
- 对自由文本的虚假成功、隐含建议、多问题和泄露风险做有界检查与行为评估，不宣称通用语义验证已经实现。
...
新增规则约束：能以局部 Candidate 或 Artifact 的合法性表达的规则才属于 Guard。产品偏好不能伪装成 Validation Error。模型自报的 Response 类型或 SupportMove 不证明正文语义正确；本轮最终文本和卡片需要一起验证，替换卡片后不可保留旧介绍。
...
- 在事务失败后返回整组成功状态；逐项提交须准确返回已成功项，见技术方案 18.2。
...
4. `StateTransition` 只能由 Kernel 产生；Aggregate 可通过受限 Apply 接口执行该结果，Application 不另行决定领域变化。
5. Evidence Guard 保证来源/结构合法，不能证明开放语义；模型推断不能升级为用户确认或业务授权。
6. 同一 Conversation 同时只允许一个有效 Running Model Effect。
7. 同一 Artifact 类型只允许设计所规定数量的 Current Pending / Processing 实例。
8. 一个模型 Turn 的 Assistant Message、Artifact 和 Conversation 状态必须原子提交；正式 Task 的整组或逐项事务语义单独定义，不能混用。
9. 迟到、版本冲突、取消或 Superseded 的结果不能覆盖新状态。
10. 模型候选的 Guard / Policy 失败后不得产生正式副作用；逐项 Confirm 后续失败时，已成功项必须可恢复且准确报告。
...
| 规则类型 | 示例 | 唯一归属 |
| --- | --- | --- |
| 安全不变量 | 无 Confirm 不创建 Task | Command 定义授权，Kernel 校验状态，DB 保证唯一提交；分层落实同一不变量 |
...
8. 有与风险相称的验证方案：状态、授权和事务采用确定性验证，表达与语义采用相关多轮样本；纯文档和文案不自动新增测试。
...
新增 Runtime 产品分支 = 0
新增 Kernel 状态 = 0
涉及行为决策时至少一个有意义的边界验证；纯文档不要求新增测试
```
...
1. 不能由现有通用字段表达。
2. 至少支持一类长期语义，而不只是一个示例句；优先表达作用域、引用、增删改和不确定性。
...
- 不访问数据库。
- 不调用模型。
- 不读取系统时间；本 Turn 固定的参考时间与时区由参数传入，重试不重新解释“明天”。
- 不修改输入对象。
- 相同输入产生相同结果。
- 返回稳定 Reason。
...
Schema 修正与 Payload 修正必须区分。锁定只在同一 Effect、BaseVersion、Frame、用户消息和权限仍有效时成立；有新消息、取消、权限撤销或过期即不能继续提交。Schema 首次解析失败时尚不存在可信候选，下一轮仍需重新执行完整的 Evidence、Readiness 和 Post-Policy 流程。Response、Strategy 或 Proposal Payload 被拒绝时，如果本轮 Interpretation 已经通过 Evidence Guard，并已经形成 Planning Decision，则 Runtime 必须锁定这两个不可变控制结果。
...
- 首次候选尚未通过 Schema；
- Evidence Guard 报告会影响请求、拒绝、目标或约束的无效声明；
...
新增能力先说明用户可见行为，再选择最小 Module。不要先为未来所有能力建立 Registry、DSL 或服务。Interface 必须包含输入、有效范围、不变量、失败结果和版本条件；内部 Calculator / Guard 可以是 Runtime 的私有组合，不要求每个类都暴露独立公共入口。

任何会话行为修改至少回答：

| 维度 | 需要明确的契约 |
| --- | --- |
| 当前需求 | 用户是在叙述、请求观点、请求建议还是安排；混合表达怎样保留 |
| 更新范围 | 修改哪个 Item / 字段 / 问题 / 偏好；对象不唯一如何处理 |
| 状态 | 只在成功提交后记问题预算和 Mutation；失败/取消不消费第二次预算 |
| 当前草案 | 普通对话仍可继续；修改和替换使用版本，不覆盖客户端未提交编辑 |
| 拒绝与恢复 | 拒绝草案不自动重新生成；暂停不等于关闭；新请求能继续会话 |
| 默认值 | 来源可见、不覆盖明确约束、不能验证的约束不交给默认生成器猜 |
| 失败 | 同时更新正文与卡片，fallback 不绕过用户限制；保留输入并允许对账 |

完整行为矩阵以技术方案 6.3–6.5、24.2 为准，不在每个 Mode 重写。规划是否就绪与本轮是否应该展示草案是两个决定；普通倾听、观点和建议不必创建 PlanningIntent。

偏好默认按当前 Turn 生效，持续偏好才进入 Snapshot；限制问题与限制建议应分别表达。不要将一次 WantsAdvice 或 WantsPause 保存为永久模式。新增字段确实需要跨 Turn 恢复时再持久化，派生状态不得为 Prompt 方便重复保存。

## 13. 测试与验收要求

新增行为的验证应与规则所属层一致。本表是实施时选取验证的地图，不要求本次文档审查编写测试；测试修改仍遵守项目本地 skill 和用户要求：
...
模型行为验证和确定性规则验证互补。固定候选输入的 Policy 测试不能证明模型理解了真实用户，模型成功样例也不能证明幂等/并发。关键规则验证命中、不命中和冲突；自然语言评估使用多轮变体并允许多种合适回复，不用精确字符串作为质量标准。

一次 Question 字段、自报 SupportMove、真实 Quote 都只是可检查的协议事实；必须另外评估否定范围、指代消解、建议是否受请求、重复提问和恢复连贯性。详见技术方案 24.2。
...
8. 执行与改动风险相称的验证；需要新增测试时遵守项目规则，文档调整不自动新增代码测试。
...
```text
模型负责开放语义和候选质量
Evidence Guard 负责验证来源与结构，并显式保留语义局限
