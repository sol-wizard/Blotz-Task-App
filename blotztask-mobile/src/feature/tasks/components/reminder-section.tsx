...
import { useReminders, ReminderTask } from "../hooks/useReminders";
...
function isTodo(task: ReminderTask): boolean {
...
const isInProgress = (task: ReminderTask) => {
...
export function ReminderSection() {
  ...
  const { filteredTasks, counts } = useMemo(() => {
    const todoTasks: ReminderTask[] = [];
    const inProgressTasks: ReminderTask[] = [];
    const doneTasks: ReminderTask[] = [];
    ...
    const statusMap: Record<TaskStatusType, ReminderTask[]> = {
      All: reminders,
      "To Do": todoTasks,
  ...
  }, [reminders, selectedStatus]);

  if (isLoading) {
    return <LoadingScreen />;
  }

  const renderReminder = ({ item }: { item: ReminderTask }) => <ReminderCard reminder={item} />;
