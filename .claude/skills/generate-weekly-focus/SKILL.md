...
6. **`--limit 30` isn't always enough — PRs roll off the list as more merge.** If the cycle since the last release has more than ~10 merges, a PR from early in that window can fall outside a 30-item cap even though it's well within the date range you're filtering for (this happened: a real merged, user-facing PR silently disappeared from the enumeration this way). After filtering by date, confirm the *oldest* PR in your result set is comfortably before the last-release cutoff — if it's close to the cap, re-run with a higher `--limit` (60+) to be sure nothing rolled off.
...
- **Every item in every section gets a topic tag, including inside the Next Release fold — this is settled, not optional.** It drifted once: a run built the fold as flat "New features"/"Bug fixes" headers with no tags at all, while the status buckets kept theirs, so the same report looked inconsistent depending on which section you were reading. The user confirmed explicitly that tags belong everywhere for visual consistency. Markup for a fold item is identical in shape to a status-bucket item — `<li><span class="tag" data-lang="en">X</span><span class="tag" data-lang="zh">Y</span><span data-lang="en">...</span><span data-lang="zh">...</span></li>` — styled via `.fold-body .tag` (always the green `--merged` wash, since every fold item shares that one status). Only the muted "Also fixed" line stays tag-free, in every section, because it's a footnote, not an item.
